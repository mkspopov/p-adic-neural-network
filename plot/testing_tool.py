    ...
    def Judge(self):
        ...
        for _ in range(len(self.params)):
            ...
            if isinstance(contestant_input, str):
                ...
                ACCURACIES.append(contestant_input)
                # self.io.PrintOutput(_CORRECT_MSG)
                return None
            if isinstance(contestant_input, float):
                ACCURACIES.append(contestant_input)

            # if i == self.max_inspection:
            #     return _ERROR_MSG_MAX_INSPECTION_EXCEED

            self.io.PrintOutput(self.Next())
...
def JudgeAllCases(test_number, io):
    ...
    params = [dict(("num_digits", 2)), dict(("num_digits", 3))]
    io.PrintOutput('{}'.format(NUM_CASES))
    for case_number in range(1, NUM_CASES + 1):
        single_case = JudgeSingleCase(params, io)
